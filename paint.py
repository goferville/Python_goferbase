...
class Paint:
    ...
    def paint_info(self):
        ...
        self.tft.circle(dx * 5 + ((dx - 2) // 2), dy + ((dispy - dy) // 2), (dispy - dy) // 2 - 2, self.tft.LIGHTGREY,
                        self.tft.LIGHTGREY)
                    ...
                    if touch and (abs(x - lx) < 4) and (abs(y - ly) < 4):
                        if (y < 20):
                            ...
                            if x > (dx * 7):
                                ...
                                if drw == 1:
                                    self.tft.circle(dx * 5 + ((dx - 2) // 2), dy + ((dispy - dy) // 2),
                                                    (dispy - dy) // 2 - 2, self.tft.LIGHTGREY, self.tft.LIGHTGREY)
                                elif drw == 3:
                                    self.tft.rect(dx * 5 + 6, dy + 2, dx - 14, dispy - dy - 4, self.tft.LIGHTGREY,
                                                  self.tft.LIGHTGREY)
                                elif drw == 2:
                                    self.tft.circle(dx * 5 + ((dx - 2) // 2), dy + ((dispy - dy) // 2),
                                                    (dispy - dy) // 2 - 2, self.tft.YELLOW, self.tft.DARKGREY)
                                elif drw == 4:
                                    self.tft.rect(dx * 5 + 6, dy + 2, dx - 14, dispy - dy - 4, self.tft.YELLOW,
                                                  self.tft.DARKGREY)
                            # drawing size
                            elif x > (dx * 4):
                                r = 10
