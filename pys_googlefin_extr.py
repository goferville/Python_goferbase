"""
    https://github.com/spidezad/google_screener_data_extract/blob/master/google_screener_data_extract/jsonwebretrieve.py
...
class GoogleStockDataExtract(object):
    ...
    def convert_json_to_df(self):
        ...
        for n in json_raw_data['searchresults']:
            temp_stock_dict = {'SYMBOL': n['ticker'], 'CompanyName': n['title'], }
    ...
    def retrieve_all_stock_data(self):
        ...
        for temp_url_mid in self.mid_url_list:
            ...
            if len(self.result_google_ext_df) == 0:
                ...
                self.result_google_ext_df = pandas.merge(self.result_google_ext_df, temp_data_df,
    ...
    def rename_columns(self):
        ...
        self.result_google_ext_df['TotalDebtToEquityYear'] = self.result_google_ext_df[
            'TotalDebtToEquityYear'].str.replace(',', '')
        self.result_google_ext_df['TotalDebtToEquityYear'] = self.result_google_ext_df['TotalDebtToEquityYear'].astype(
            'float')

        self.result_google_ext_df = self.result_google_ext_df.rename(
            columns={'CompanyName': 'GS_CompanyName', 'AverageVolume': 'GS_AverageVolume', 'Volume': 'GS_Volume',
                     'AINTCOV': 'Interest_coverage', 'DividendYield': 'TRAILINGANNUALDIVIDENDYIELDINPERCENT',
                     'PE': 'PERATIO', 'TotalDebtToEquityYear': 'TotalDebtEquity', 'PriceToBook': 'PRICEBOOK',
